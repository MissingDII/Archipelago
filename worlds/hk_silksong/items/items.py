...
from ..data.items_locations_data import LocationItemData
...
def create_items(item_factory: SilksongItemFactory, world_options: SilksongOptions, locations_items_pairs: Dict[str, LocationItemData], enabled_locations: list[str],
                 excluded_items: list[str], random: Random) -> List[Item]:
    items_to_create = []
    items_to_create.extend(choose_items_based_on_enabled_locations(item_factory, world_options, locations_items_pairs, enabled_locations))
...
def choose_items_based_on_enabled_locations(item_factory: SilksongItemFactory, world_options: SilksongOptions, locations_items_pairs, enabled_locations) -> List[str]:
    items = []
    for enabled_location in enabled_locations:
        ...
        if loc_item_pair.item is None:
            continue
        items.append(loc_item_pair.item.name)
    return items
